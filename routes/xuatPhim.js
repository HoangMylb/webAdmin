...
router.post('/:id/edit',async function(req, res, next) {
    ...
    try {
        //lấy giá trị name từ body
        let { Phong,ngay,gio} = req.body;
            let ojDB = {
                Phong:Phong,ngay:ngay,gio:gio           
            }
...
router.get('/getXuatPhim', async (req, res, next) => {
    let { phim, rap } = req.query; // Sửa từ req.body thành req.query
  
    try {
      let xuatPhim = await XuatPhimController.getXuatPhim(phim, rap);
      if (xuatPhim.success) {
        res.status(200).json({
          success: true,
          message: xuatPhim.message
        });
      } else {
        res.status(400).json({
          success: false,
          message: 'Không tìm thấy thông tin xuất phim'
        });
      }
    } catch (error) {
      console.error(error);
